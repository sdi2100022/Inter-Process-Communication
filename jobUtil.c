...
void jobtuple_constructor(jobtuple *tuple, char *jobID, char *job){
...
void concatenated_string(void *data, char **result) {
    jobtuple *tuple = (jobtuple*)data;
    int buffer_size = snprintf(NULL, 0, "<%s,%s,%d>\n", tuple->jobID, tuple->job, tuple->queuePosition);
    ...
    snprintf(buffer, buffer_size + 1, "<%s,%s,%d>\n", tuple->jobID, tuple->job, tuple->queuePosition);
